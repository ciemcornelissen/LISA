...
# This makes Python look for modules inside 'src' as if they were top-level.
# os.path.dirname(__file__) gets the directory of the current script.
# os.path.abspath() makes it an absolute path.
# We are assuming your utilities.py is in the 'src' folder.
# If pipeline.py is in 'src', we need to go up one level.
project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
sys.path.insert(0, project_root)
# Now Python can find 'src.utilities' when the script asks for it,
# AND pickle can find 'utilities' because we've effectively added the
# 'src' directory's contents to the top-level search path.

# Let's adjust for your exact structure. If pipeline.py is in src, and utilities.py is in src,
# and you run from the project root, the import should be `from src import utilities`.
# The problem is that pickle expects `utilities`. So we add the src folder to the path.
...
def load_models_data():
    ...
    weight_config_params = torch.load('models/cross_weight_predictor_epoch_99_R2_0.757_end_R20.610_runID_i0mijofs.pth',
    ...
    # yolo_model = YOLO(f'models/yoloModel_wandb_vb5075nk_map50_0.7878.pt', task='detect')
    yolo_model = YOLO(f'models/yoloModel_wandb_5l8pg9sf_map50old_0.5658.pt', task='detect')
    ...
    predictor.load_state_dict(torch.load('models/ae_predictor_epoch_150_R2_0.5951396226882935_runID_cd7605a0.pth',
                                         map_location=torch.device('cpu'))['sugar_predictor_state_dict'])
    predictor.eval()
    AE.load_state_dict(torch.load('models/ae_predictor_epoch_150_R2_0.5951396226882935_runID_cd7605a0.pth',
    ...
    with open('models/weight_images.pkl', 'rb') as f:
        weight_images = pickle.load(f)
    print("'weight_images' loaded.")

    with open('models/images.pkl', 'rb') as f:
