...
5. Download the pretrained model bundle (fetches YOLO checkpoints, transformer weights, and cached datasets):
   ...
   ```
   This pulls the hosted archive, extracts its contents into `models/`, and keeps the directory ready for inference.
...
If you maintain a private mirror, override the source URL (and optional archive filename):

```bash
LISA_MODELS_URL="https://example.com/custom_models.tar.gz" \
LISA_MODELS_ARCHIVE_NAME="custom.tar.gz" \
bash scripts/download_models.sh
```
